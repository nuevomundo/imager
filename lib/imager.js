...
Imager.prototype = {
  ...
  zoomCropFile: function (file, preset, filename, cb) {
    ...
    var zoom = (preset.args.zoom === undefined) ? '1.0' : preset.args.zoom;
    var offset = (preset.args.offset === undefined) ? '0,0' : preset.args.offset;

    var dimensions = imageSize(file['path']);
    console.log(dimensions);
    var resize = function (dimension) {
      return Math.ceil(dimension * parseFloat(zoom));
    };

    gm(file['path'])
      .resize(resize(dimensions.width), resize(dimensions.height))
      .crop(preset.type.crop.split('x')[0], preset.type.crop.split('x')[1],
        offset.split(',')[0], offset.split(',')[1])
      .write(tempFile, function (err) {
        if (err) return cb(err);
        async.each(self.storage, function (storage, cb) {
          self['pushTo' + storage](tempFile, remoteFile, filename, ct, cb);
