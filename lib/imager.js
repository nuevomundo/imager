...
Imager.prototype = {
  ...
  upload: function (files, callback, variant, args) {
    var self = this;
    ...
    async.map(files, getFileInfo, function (err, files) {
      ...
      var prepare = function (file, fn) {
        ...
        self.prepareUpload(file, filename, variant, fn, args);
      };
  ...
  prepareUpload: function (file, filename, variant, fn, args) {
    ...
    if (variant.zoomAndCrop) {
      Object.keys(variant.zoomAndCrop).forEach(function (name) {
        var processFiles = function (cb) {
          var preset = {
            name: name,
            type: variant.zoomAndCrop[name],
            sep: variant.separator || '_',
            args: (args === undefined) ? {} : args
          };
          self.zoomCropFile(file, preset, filename, cb);
        };
        asyncArr.push(processFiles);
      });
    }
  ...
  zoomCropFile: function (file, preset, filename, cb) {
    var self = this;
    var ct = getFileType(file);
    var remoteFile = preset.name + preset.sep + filename;
    var tempFile = path.join(tempDir, 'imager_' +
    ...
    var container = (preset.args.container === undefined) ? '0,0' : preset.args.container.split('x');
    var zoom = (preset.args.zoom === undefined) ? '1.0' : parseFloat(preset.args.zoom);
    var offset = (preset.args.offset === undefined) ? '0,0' : preset.args.offset.split(',');
    var crop = preset.type.split('x');

    var applyZoomCrop = function (gmFile) {
      if (crop.length > 1) {
        // Find the zoom on the constraining (smaller) dimension
        var ratio = [crop[1] / container[1], crop[0] / container[0]];
        var i = (ratio[1] > ratio[0]) ? 1 : 0;
        var extraZoom = ratio[i];
        // center the crop on the larger dimension if necessary
        offset[i] = Number(offset[i]) + (container[i] - crop[i] / extraZoom) / 2;

        offset = [offset[0] * extraZoom, offset[1] * extraZoom];

        var dims = imageSize(file['path']);
        var w = Math.ceil(dims.width * zoom * extraZoom);
        var h = Math.ceil(dims.height * zoom * extraZoom);

        return gmFile.resize(w, h).crop(crop[0], crop[1], offset[0], offset[1]);
      } else {
        return gmFile.resize(crop[0]);
      }
    };
    applyZoomCrop(gm(file['path']))
      .write(tempFile, function (err) {
        if (err) return cb(err);
        async.each(self.storage, function (storage, cb) {
          self['pushTo' + storage](tempFile, remoteFile, filename, ct, cb);
