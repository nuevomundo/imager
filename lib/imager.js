...
Imager.prototype = {
  ...
  zoomCropFile: function (file, preset, filename, cb) {
    ...
    var applyZoomCrop = function (gmFile) {
      if (crop.length > 1) {
        var extraZoom = [crop[0] / container[0], crop[1] / container[1]]

        offset[0] = offset[0] * extraZoom[0];
        offset[1] = offset[1] * extraZoom[1];

        var dims = imageSize(file['path']);
        var w = Math.ceil(dims.width * zoom * extraZoom[0]);
        var h = Math.ceil(dims.height * zoom * extraZoom[1]);
